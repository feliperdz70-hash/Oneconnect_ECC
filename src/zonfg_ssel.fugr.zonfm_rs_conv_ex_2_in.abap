function ZONFM_RS_CONV_EX_2_IN.
*"--------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"     REFERENCE(NO_TCURX) DEFAULT SPACE
*"  EXPORTING
...
*"--------------------------------------------------------------------
  ...
  if currency ne space.
    ...
    elseif sy-subrc <> 0 and no_tcurx = 'X'.
  ...
  if currency ne space and l_convert-decimals = 0.
    ...
    perform convert_ex_2_in(rsdynss0) using    l_convert
                                      ...
                                      changing l_subrc l_error
                                               <l_f>.
    if l_convert-convexit is initial.
      l_exp = 10 ** l_convert-dddecimals .
      output_internal = <l_f> / l_exp .
    else.
      output_internal = <l_f>.
    endif.
  else.
